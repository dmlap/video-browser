...
async function buildMap (componentsPath, outputName) {
  const components = []
  const entries = (
    await fs.readdir(componentsPath, {
      encoding: 'utf8',
      withFileTypes: true
    })
  ).map((entry) => [entry, '.'])
  ...
  try {
    ...
    for (let [entry, currentPath] = entries.pop(); entries.length > 0; [entry, currentPath] = entries.pop()) {
      ...
      if (entry.isFile() && /^\.(js|mjs)$/i.test(path.extname(entry.name))) {
        ...
        const symbol = entry.name
          .replaceAll(path.delimiter, '_slash_')
          .replaceAll('.', '_dot_')
          .replaceAll('-', '_dash_')
      ...
      if (entry.isDirectory()) {
        for (const newEntry of await fs.readdir(path.join(componentsPath, currentPath))) {
    ...
    for (const [path, symbol] of components) {
